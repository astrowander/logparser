...
public:
	...
	void OutputStatistics()
    {
        ...
        std::map<uint32_t, std::set<std::string>> topDomains;
        for (auto it = _domains.begin(); it != _domains.end(); ++it)
        {
			auto topDomainsIt = topDomains.find(it->second);
			if (topDomainsIt == topDomains.end())
			{
				topDomains.insert(std::make_pair(it->second, std::set<std::string>({ it->first })));
				continue;
			}
        ...
        std::map<uint32_t, std::set<std::string>> topPaths;
